...
- Citation validation failure: REVIEW, status pending, rule `R_CITATION_INVALID`, `reason_code` null (no new reason code added to the fixed vocabulary). An exception while deciding one charge (or in the pre-checks, which then affects every charge): REVIEW, pending, rule `R_ENGINE_ERROR`, and the charge is still persisted. `reason_code` is `MODEL_UNAVAILABLE` when the failure is a dependency (database) error, per rule 5; an engine bug carries no reason code and the error text is in the reason. Only `final` upstream records can settle a charge; `pending` and `overridden` ones count as uncertain.
