...
values to PASS / FAIL / UNCERTAIN, the same records the agent reads) and the sourced
channel rules for deadlines. Nothing here imports the decision path (app.engine,
...
from app.adapters.csv_v0 import load_fee_report, load_upstream
...
def deadline_text(c: Charge, rules: ChannelRules) -> str:
    ...
    since = f"counted from the posted date {c.posted_date}"
...
# Raw values that only repeat the verdict add nothing for a labeller.
_REDUNDANT_DETAILS = {"yes", "no", "uncertain", "pass", "fail"}


def _check_text(key: str, verdict: str, detail: str | None) -> str:
    name = CHECK_NAMES.get(key, _plain(key))
    if not detail or detail.lower() in _REDUNDANT_DETAILS:
        return f"{name} {verdict}"
    return f"{name} {verdict} ({_plain(detail).replace('=', ': ')})"
...
def record_text(r: EvidenceRecord) -> str:
    ...
    checks = [_check_text(c.check_key, c.verdict.value, c.detail) for c in r.checks]
...
def summary(
    c: Charge, records: list[EvidenceRecord], others: list[Charge], rules: ChannelRules
) -> str:
    lines: list[str] = []
    mine = sorted(
    ...
    if not mine:
        ...
        lines.extend(record_text(r) for r in mine)
        ...
        rows.append(
            {
                ...
                "evidence_summary": summary(c, org_records, others, rules),
                "label": "",
                "reason": "",
            }
        )
